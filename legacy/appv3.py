...
class CourseManagerApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Postać anime (bez wpływu na layout)
    ...
    def create_left_menu(self):
        ...
        menu_buttons = [
            ...
            ("Pełny ekran", self.toggle_fullscreen)  # Przycisk do testowania pełnego ekranu
        ]
    ...
    def update_anime_character(self):
        if self.current_style == "Sakura Pink" and os.path.exists("anime_character.png"):
            ...
            self.anime_character_label.move(
                self.main_area.width() - 200 - 10,  # 10 pikseli marginesu
                self.main_area.height() - 300 - 10
            )
    ...
    def resizeEvent(self, event):
        # Aktualizacja pozycji postaci anime przy zmianie rozmiaru (choć rozmiar jest stały)
        if self.current_style == "Sakura Pink" and self.anime_character_label.isVisible():
            self.anime_character_label.move(
    ...
    def toggle_fullscreen(self):
        if self.isFullScreen():
            ...
            self.showFullScreen()
        # Aktualizacja pozycji postaci po zmianie trybu
        self.update_anime_character()
    ...
    def add_grade(self):
        ...
        if self.editing_grade_index is not None:
            try:
                ...
                logging.info(f"Zaktualizowano ocenę w kursie: {course['name']}, indeks: {self.editing_grade_index}")
            ...
            logging.info(f"Dodano ocenę do kursu: {course['name']}")
    ...
    def on_grade_selection_changed(self):
        ...
        if selected_items:
            self.edit_grade_btn.setEnabled(True)
            self.remove_grade_btn.setEnabled(True)
        else:
            self.edit_grade_btn.setEnabled(False)
            self.remove_grade_btn.setEnabled(False)
    ...
    def remove_grade(self):
        ...
        try:
            ...
            self.grade_date_input.setDate(QDate.currentDate())
            self.editing_grade_index = None
            logging.info(f"Usunięto ocenę z kursu: {course['name']}, indeks: {item_index}")
        except IndexError as e:
