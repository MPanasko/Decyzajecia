...
class CourseManagerApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        opacity_effect_btn = QGraphicsOpacityEffect()
        opacity_effect_btn.setOpacity(0.5)
        self.analyze_btn.setGraphicsEffect(opacity_effect_btn)
        ...
        # Wywołanie update_anime_character po utworzeniu analyze_btn
    ...
    def update_anime_character(self):
        if self.current_style == "Sakura Pink" and os.path.exists("anime_character.png"):
            ...
            # Efekt przezroczystości dla postaci
            opacity_effect = QGraphicsOpacityEffect()
            opacity_effect.setOpacity(0.7)
            self.anime_character_label.setGraphicsEffect(opacity_effect)
            # Pozycjonowanie w prawym dolnym rogu
            x_pos = self.main_area.width() - 200 - 10
            y_pos = self.main_area.height() - 300 - 50  # 50 pikseli na przycisk analizy
            self.anime_character_label.move(x_pos, y_pos)
            self.anime_character_label.lower()  # Pod UI
            self.stacked_widget.raise_()  # UI nad postacią
            if hasattr(self, 'analyze_btn'):
                self.analyze_btn.raise_()  # Przycisk analizy nad postacią
            self.anime_character_label.show()
            logging.info(f"Postać anime załadowana, pozycja: ({x_pos}, {y_pos})")
        else:
            self.anime_character_label.hide()
    ...
    def update_name(self):
        ...
        self.save_data()
        self.update_home_page()
        logging.info(f"Zaktualizowano imię: {name}")
    ...
    def update_grades_form(self):
        ...
        # Znajdź kurs w self.courses na podstawie nazwy
        course = None
    ...
    def add_grade(self):
        ...
        # Zaktualizuj self.courses
        self.courses[course_index] = course
        self.save_data()
        self.update_courses_list()  # Aktualizuje course_combo
        # Ustaw ponownie wybrany kurs
    ...
    def apply_style(self, style_name):
        ...
        styles = {
            "Sakura Pink": {
                ...
                "button_color": "#ffb6c1",
                "button_text": "#5a0032",
                "list_color": "rgba(255, 209, 220, 0.5)",  # 50% przezroczystości
                "group_color": "rgba(255, 236, 241, 0.5)",  # 50% przezroczystości
                "menu_color": "#ffc0cb",
                "scroll_color": "rgba(255, 236, 241, 0.5)"  # 50% dla QScrollArea
            },
            "Windows XP": {
                "bg_color": "#e4ecf5",
                "text_color": "#003366",
        ...
        # Aktualizacja efektów przezroczystości w zależności od stylu
        if style_name == "Sakura Pink":
            opacity_effect = QGraphicsOpacityEffect()
            opacity_effect.setOpacity(0.5)
            self.stacked_widget.setGraphicsEffect(opacity_effect)
            if hasattr(self, 'analyze_btn'):
                self.analyze_btn.setGraphicsEffect(opacity_effect)
        else:
